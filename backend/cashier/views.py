...
class CashierSessionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Filter by status if provided (OPEN/CLOSED)
        status_val = (qp.get("status") or "").upper()
        if status_val in ("OPEN", "CLOSED"):
            qs = qs.filter(status=status_val)
        # Date range filters (ISO format YYYY-MM-DD). Apply to opened_at/closed_at dates
        date_from = qp.get("date_from")
        date_to = qp.get("date_to")
        from datetime import datetime
        if date_from:
            try:
                qs = qs.filter(opened_at__date__gte=datetime.fromisoformat(date_from).date())
            except Exception:
                pass
        if date_to:
            try:
                qs = qs.filter(closed_at__date__lte=datetime.fromisoformat(date_to).date())
            except Exception:
                pass
        # Ordering support via ?ordering=field (defaults handled by DRF ordering filter or queryset order)
