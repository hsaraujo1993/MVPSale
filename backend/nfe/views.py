...
class NFeInvoiceViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def download_danfe(self, request, pk=None):
        ...
        if result.get("http_status") != 200 or not content:
            return Response({"detail": "Falha ao obter DANFE", "provider_status": result.get("http_status")}, status=status.HTTP_400_BAD_REQUEST)
        resp = HttpResponse(content, content_type=result.get("content_type", "application/pdf"))
        resp["Content-Disposition"] = f"inline; filename=danfe-{inv.chave}.pdf"
        return resp
